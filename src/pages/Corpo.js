...
const Ingresso = ({date}) => {
  ...
  React.useEffect(() => {
    const date = atob(window.location.href.split('http://localhost:3000/lagoa/#/ingressos/')[1]);
   
    requisicaoGetApi(`https://sofalta.eu/api/v4/empreendimentos/lagoa/produtos/ingressos/ingressos?data=${date.split('-').reverse().join('-')}`);
  }, []);
